...
class LoginController extends Controller
{
	...
	function actionTwittercomplete()
	{
	    if(Yii::app()->request->isAjaxRequest)
	    {
            ...
            if($user = User::model()->findByAttributes(array('email' => $_POST['email']))){
                Yii::app()->session['tw_verify'] = $user->id;
                /*if ($user->verified_tw == 1) {
                	$data['error'] = 1;
            ...
            if($data['error'] == 0)
            {
	            ...
	            if($user)
	            {
		            ...
		            $user->verified_tw = 1;
		            $user->id_tw = Yii::app()->session['id_tw'];
		            $user->save();
	...
	function actionTwitter()
	{
		...
		if(!is_null($user))
        {
			...
			Yii::app()->session['tw_verify'] = $user->id;
			Yii::app()->session['id_tw'] = $userProfile->identifier;
	...
	function actionFacebook()
	{
		$config = array( 
			...
			"providers"  => array (
				"Facebook"   => Yii::app()->params['hoauth']['providers']['Facebook'],
			),		 
		);
 	    if(isset($_REQUEST['access_token']))
        {
