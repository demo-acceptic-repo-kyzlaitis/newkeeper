...
class UserModule extends CWebModule
{
	...
	public $returnLogoutUrl = array("/news");
    ...
    static private $_moder;
    static private $_corr;
	...
	{
	    //$this->layoutPath = Yii::getPathOfAlias('views.layouts');
        //var_dump($this->layoutPath);exit;
	...
	public static function isWithHighPermissions()
    {
        if(self::isModer() || self::isCorr() || self::isAdmin())
            return true;
	}
	 ...
	 * @return boolean
	 */
	public static function isCorr() {
		if(Yii::app()->user->isGuest)
			return false;
		else {
			if (!isset(self::$_corr)) {
			    $roles = Rights::getAssignedRoles(Yii::app()->user->Id);
                if(isset($roles['Corrector']))
					self::$_corr = true;
				else
					self::$_corr = false;
			}
