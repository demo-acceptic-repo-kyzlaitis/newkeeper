...
class Profile extends UActiveRecord
{
    ...
    const AVATAR_SIZE = 'uploads/avatars/';
		...
		if (!$this->_rules) {
			...
			foreach ($model as $field) {
				...
				if ($field->range) {
					...
					if ($field_rule) $field_rule['message'] = UserModule::t($field->error_message);
					array_push($rules,$field_rule);
				}
				if($field['varname'] == 'phone')
				{
					$field->error_message = 'Check phone';
				}
			}
	...
	public function attributeLabels()
	{
		...
		foreach ($model as $field)
			$labels[$field->varname] = ((Yii::app()->getModule('user')->fieldsMessage)?UserModule::t(Yii::t('app',$field->title),array(),Yii::app()->getModule('user')->fieldsMessage):Yii::t('app',$field->title));
    ...
    {
        return "/uploads/avatars/".$this->avatar_source;
    }
}