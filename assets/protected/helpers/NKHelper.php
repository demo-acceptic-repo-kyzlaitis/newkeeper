...
class NKHelper
{
    ...
    public function getBkmrkProvider()
    {
        if(Yii::app()->user->isGuest){
    ...
    public function getBlogProvider($id)
    {
		$dataProvider=new CActiveDataProvider('News', array(
			'criteria' => array(
	...
	public function bookmarkStat($id="")
	{
    ...
    public function shareStat($id, $soc)
	{
    ...
    public function downloadStat($id)
    {
		$stat = StatisticsOperation::model()->findByPk($id);
		if ($stat) {
...
//var_dump($str_length);exit;
        ...
        if ($str_length <= $limit_str_length) {
            ...
            // www.newskeeper.net
            imagettftext($im,$font_size_nk,0, $cright_x, $cright_y, $color, $font_path, $nk_text);
            // Линия
