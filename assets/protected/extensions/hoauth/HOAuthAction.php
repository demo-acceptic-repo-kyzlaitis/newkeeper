	...
	protected function sendActivationMail($user)
	{
		if($this->useYiiUser)
		{
			...
			if (Yii::app()->getModule('user')->sendActivationMail) 
			{
				...
				$passwd = rand(11111111,99999999);
				//$user = User()::model()->findByPk(Yii::app()->user->id);
				$user->password = md5($passwd);
				if (!$user->save())
					die("password not saved!");
				if (Yii::app()->language == "en"){
					UserModule::sendMail($user->email,UserModule::t("You registered on {site_name}",array('{site_name}'=>Yii::app()->name)),UserModule::t("Login: '{username}'<br>Password: '$passwd'<br> To change password go to http://".$_SERVER['HTTP_HOST']."/user/profile/edit ",array('{username}'=>$user->username)));
				}elseif(Yii::app()->language == "ru"){
					UserModule::sendMail($user->email,UserModule::t("Вы зарегестрировались на {site_name}",array('{site_name}'=>Yii::app()->name)),UserModule::t("Логин: '{username}'<br>Пароль: '$passwd'<br> Чтоб изменить пароль пройдите http://".$_SERVER['HTTP_HOST']."/user/profile/edit ",array('{username}'=>$user->username)));
				
				}else{
					UserModule::sendMail($user->email,UserModule::t("Ви зареєструвались на {site_name}",array('{site_name}'=>Yii::app()->name)),UserModule::t("Логін: '{username}'<br>Пароль: '$passwd'<br> Щоб змінити пароль пройдіть http://".$_SERVER['HTTP_HOST']."/user/profile/edit ",array('{username}'=>$user->username)));
				}
