<?php $this->breadcrumbs = array(
	'Права'=>Rights::getBaseUrl(),
	Rights::t('core', 'Сгенерировать элементы'),
); 
$this->pageTitle='Сгенерировать элементы';?>

<div id="generator">

	<p><?php echo Rights::t('core', 'Выберите какие элементы вы хотите наследовать.'); ?></p>
<div class="row-fluid">
    <div class="widget">
        <div class="head">
            <div class="icon"><i class="icosg-clipboard1"></i></div>
        </div>                
            <div class="block-fluid">
	<div class="form">

		<?php $form=$this->beginWidget('CActiveForm'); ?>

			<div class="row">

				<table class="items generate-item-table" border="0" cellpadding="0" cellspacing="0">

					<tbody>

						<tr class="application-heading-row">
							<th colspan="3"><?php echo Rights::t('core', 'Приложение'); ?></th>
						</tr>

						<?php $this->renderPartial('_generateItems', array(
							'model'=>$model,
							'form'=>$form,
							'items'=>$items,
							'existingItems'=>$existingItems,
							'displayModuleHeadingRow'=>true,
							'basePathLength'=>strlen(Yii::app()->basePath),
						)); ?>

					</tbody>

				</table>

			</div>

			<div class="row">

   				<?php echo CHtml::link(Rights::t('core', 'Select all'), '#', array(
   					'onclick'=>"jQuery('.generate-item-table').find(':checkbox').attr('checked', 'checked');
                       jQuery('.generate-item-table').find(':checkbox').parent().addClass('checked'); return false;",
   					'class'=>'selectAllLink')); ?>
   				/
				<?php echo CHtml::link(Rights::t('core', 'Select none'), '#', array(
					'onclick'=>"jQuery('.generate-item-table').find(':checkbox').removeAttr('checked');
                    jQuery('.generate-item-table').find(':checkbox').parent().removeClass('checked'); return false;",
					'class'=>'selectNoneLink')); ?>

			</div>

   			<div class="toolbar bottom TAR">

				<?php echo CHtml::submitButton(Rights::t('core', 'Сгенерировать'),array('class'=>'btn btn-primary')); ?>

			</div>

		<?php $this->endWidget(); ?>

	</div>
</div>
</div>
</div>
</div>