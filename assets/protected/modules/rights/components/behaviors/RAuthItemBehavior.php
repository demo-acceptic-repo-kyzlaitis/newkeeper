...
class RAuthItemBehavior extends CBehavior
{
	...
	*/
	public function getRevokePermissionLink(CAuthItem $role)
	{
		...
		return CHtml::link(Rights::t('core', 'Отозвать'), '#', array(
			'onclick'=>"
				jQuery.ajax({
					type:'POST',
	...
	*/
	public function getAssignPermissionLink(CAuthItem $role)
	{
		...
		return CHtml::link(Rights::t('core', 'Назначить'), '#', array(
			'onclick'=>"
				jQuery.ajax({
					type:'POST',
	...
	*/
	public function getInheritedPermissionText($parents, $displayType=false)
	{
		...
		return '<span class="inherited-item" title="'.implode('<br />', $items).'">'.Rights::t('core', 'Унаследовано').' *</span>';
	}
}
